...
setwd("Z:/cz1113/Chapter_2/Chapter2_MC/20220218")
...
rq.fit.PanelIE <- function(AX_T,AY,tau=TAU,R,iter=200,cri_V=0.00001,c1,power1,smooth){
  ...
  #quantile and individual specific coefficient for factor loading
...
Xphi<-0.5#autoregressive parameter for error in x 
Fphi<-0.5#autoregressive parameter for factors 
...
BN<-15 #length of burn-in time period 
TaB<-T+BN #Total time period
TAU <- 0.25#quantile
MC<-200#Number of MCs
exp<-3 #experiment indicator
dis<-1#distribution indicator
...
fl_x_mean<-0.5
fl_x_var<-0.5
fl_cor<-0.5 ##we may consider smaller value

kappa1<-1 #parameter in front of the idiosyncratoc error
kappa2<-1#parameter in front of the idiosyncratoc error
...
for(T in t_range){
  ...
  TaB<-T+BN #Total time period
  ##Bandiwidth
  #hs<-T^(-1/3)*(qnorm(0.975))^(2/3)*(1.5*(dnorm(qnorm(TAU))^2)/(2*(qnorm(TAU))^2+1))^(1/3)
  #hb<-T^(-1/5)*(4.5*(dnorm(qnorm(TAU))^4)/((2*(qnorm(TAU))^2+1)^2))^(1/5)
  # h_range1<-1.3/c((N*T)^(1/13),(N*T)^(1/14))
  # h_range2<-1.5/c((N*T)^(1/13),(N*T)^(1/14))
  h_range1<-c(1/(N*T)^(1/13),1.1/(N*T)^(1/13))
  h_range2<-c(1.2/(N*T)^(1/14),1.3/(N*T)^(1/14))
  #h_range<-c(h_range,3*hs,3.5*hs,0.6*hb,0.9*hb)
  #h_range<-c(h_range,3*hs,3.5*hs)
  #h_range<-seq(0.1,0.9,0.1)
  h_range<-c(h_range1,h_range2)
...
# Estimation of Variance --------------------------------------------------
    ...
    #     hf<-h_scale*hf
    #   }
    #   # We do not use the following method when estimating the sparse density function.
    #   # for (i in 1:N){
    #   #   if((TAU+hf) >= 0.98| (TAU-hf) <= 0.02){
    #   #     density_u_tau[i,]<-2*hf/(quantile(residual_tau[,i],TAU+0.24)-quantile(residual_tau[,i],TAU-0.24))
    #   #     h2<-0.24
    #   #   } else{
    #   #     density_u_tau[i,]<-2*hf/(quantile(residual_tau[,i],TAU+hf)-quantile(residual_tau[,i],TAU-hf))
    #   #     h2<-hf
    #   #   }
    #   # }
      ...
      #h<-3*T^(-1/3)*(qnorm(0.975))^(2/3)*(1.5*(dnorm(qnorm(TAU))^2)/(2*(qnorm(TAU))^2+1))^(1/3)
      k <- function (t) {
        ifelse(abs(t)<=1,1,0)*3465/8192 *(7-105*t^2+462*t^4-858*t^6+715*t^8-221*t^10) #we need eigth order kernel function
        ###105/64 *(1-5*t^2+7*t^4-3*t^6)
      }
      k1 <- function (t) {
        ifelse(abs(t)<=1,1,0)*3465/8192 *(-105*2*t^1+462*4*t^3-858*6*t^5+715*8*t^7-221*10*t^9) #we need eigth order kernel function
        ###105/64 *(1-5*t^2+7*t^4-3*t^6)
      }
      # l2<-function (t) {
      #   ifelse(abs(t)<=1,1,0)*(2*k(t)/h+k1(t)*t/h)
      # }
      l2<-function (t) {
        ifelse(abs(t)<=1,1,0)*(k(t)/h)
      }
      F<-fit_all$F
      ...
      for(i in 1:N){
        ...
        for (t in 1:T){
          Xi_i<-Xi_i+l2(residual_tau[t,i]/h)*as.matrix(X[(i-1)*T+t,])%*%F[t,]
          Omega_i<-Omega_i+l2(residual_tau[t,i]/h)*as.matrix(F[t,])%*%F[t,]
      ...
      for (i in 1:N) {
        for(t in 1:T){
          Delta<-Delta+l2(residual_tau[t,i]/h)*as.matrix(W[(i-1)*T+t,])%*%W[(i-1)*T+t,]
        }
      }
      Delta<-Delta/(N*T)
      ...
      for (t in 1:T){
        ...
        for (j in 1:N) {
          Lambda_t<-Lambda_t+l2(residual_tau[t,i]/h)*as.matrix(W[(j-1)*T+t,])%*%loading_tau[j,]
        }
        Lambda_t<-Lambda_t/N
        for (i in 1:N){
      ...
      for(t in 1:T){
        Lambda_t<-matrix(0,p+2,R)
        for (i in 1:N) {
          Lambda_t<-Lambda_t+l2(residual_tau[t,i]/h)*as.matrix(W[(i-1)*T+t,])%*%loading_tau[i,]
        }
        Lambda_t<-Lambda_t/N
        for (i in 1:N){
    ...
    F <- PQ(X,st)$Ph##factor approximation
    dim(F)
    dim(cbind(y,X))
    F<-F[1:T,]
    ...
    cat(sprintf("\"N=%d\" \"T=%d\" \"m=%d\" \"h=%.1f\" \"tau=%.1f\" \"dis=%.1f\" \n", N, t_range[t_index],m,h_range[h_index],TAU,dis))
...
BIAS<-cbind(Bias,Bias_c,CCEBias,Bias_c_chen)
colnames(BIAS)=c("Twostep","SPJ","CCE","SPJ_chen")
...
TRMSE<-cbind(RMSE,RMSE_c,CCERMSE,RMSE_c_chen)
colnames(TRMSE)=c("Twostep","SPJ","CCE","SPJ_chen")
...
size<-cbind(size_c/MC)
row.names(size)<-c(rep("T=30",(p+1)),rep("T=50",(p+1)),rep("T=100",(p+1)))
colnames(size)<-c(rep("SPJ_c",length(h_range)))
size
power<-cbind(power_c/MC)
row.names(power)<-c(rep("T=30",(p+1)),rep("T=50",(p+1)),rep("T=100",(p+1)))
colnames(power)<-c(rep("SPJ_c",length(h_range)))
power
...
# write.csv(F_Rsquare, file= sprintf("Factor,N=%d, T=(%d,%d,%d,%d), R=%d, MC=%d,autocorr=(%.1f,%.1f,%.1f),FL_XY(%.1f,%.1f,%.1f,%.1f,%.1f),kappa=(%.1f,%.1f), (%d).csv",N,t_range[1],t_range[2],t_range[3],t_range[4],R, MC,rho,Fphi,Xphi,fl_mean,fl_x_mean,fl_var,fl_x_var,fl_cor,kappa1,kappa2,exp))
write.csv(Size_Power, file= sprintf("Size & Power,N=%d, T=(%d,%d,%d,%d),h=(%.2f,%.2f,%.2f,%.2f,%.2f,%.2f), R=%d, MC=%d,TAU=%.2f,autocorr=(%.1f,%.1f,%.1f),robust=%d, Distribution=%d,FL_XY(%.1f,%.1f,%.1f,%.1f,%.1f),kappa=(%.1f,%.1f), (%d).csv",N,t_range[1],t_range[2],t_range[3],t_range[4],h_range[1],h_range[2],h_range[3], h_range[4],h_range[5], h_range[6],R, MC,TAU,rho,Fphi,Xphi,robust,dis,fl_mean,fl_x_mean,fl_var,fl_x_var,fl_cor,kappa1,kappa2,exp))
